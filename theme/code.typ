...
#let python-lang = code-lang(
    ...
    (
        "keyword",
        (
            "def", "class", "lambda", "return", "yield", "await", "async", "if", "elif", "else", "for", "while",
            "break", "continue", "pass", "try", "except", "finally", "raise", "with", "as", "import", "from",
            "global", "nonlocal", "assert", "del", "in", "is", "not", "and", "or", "match", "case", "True",
            "False", "None", "self", "cls",
        ),
    ),
    (
        "name",
        (
            "abs", "all", "any", "bool", "bytes", "dict", "enumerate", "filter", "float", "frozenset", "getattr",
            "hasattr", "int", "isinstance", "issubclass", "iter", "len", "list", "map", "max", "min", "next",
            "object", "open", "print", "range", "repr", "reversed", "round", "set", "setattr", "sorted", "str",
            "sum", "super", "tuple", "type", "zip",
        ),
    ),
    // Ints, floats, exponents, hex/oct/bin, and `1_000_000`.
    ("number", "\\b(?:0[xX][0-9a-fA-F_]+|0[oO][0-7_]+|0[bB][01_]+|[0-9][0-9_]*(?:\\.[0-9_]*)?(?:[eE][-+]?[0-9]+)?j?)\\b"),
)
...
#let code-config = (
    ...
    font: ("Consolas", "Inconsolata", "Source Han Sans SC"),
